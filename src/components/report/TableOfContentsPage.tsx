...
export function TableOfContentsPage(props: { totalPages: number; pageMap?: Record<string, number> }) {
    ...
    <ReportPageFrame pageNumber={2} totalPages={props.totalPages}>
      ...
      <ol className="flex flex-col gap-1 text-[11px] text-slate-700">
        {filteredEntries.map((entry) => {
          const page = (entry.key && props.pageMap?.[entry.key]) ?? entry.page;
          return (
            <li
              key={entry.label}
            ...
            >
              ...
              <span className="tabular-nums text-slate-500">{page ?? ''}</span>
            </li>
          );
        })}
      </ol>
